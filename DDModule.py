...
class DDPath(nn.Module):
    def __init__(self,out_channels,kernel_size,dilation):
        ...
        self.net = nn.Sequential(
            nn.LazyConv2d(out_channels,kernel_size=1,device=self.device),
            nn.LazyBatchNorm2d(device=self.device),
            nn.LazyConv2d(out_channels,kernel_size=kernel_size,dilation=dilation,padding=(kernel_size//2)*dilation,device=self.device),
            nn.LazyBatchNorm2d(device=self.device)
        )
