...
class FastViT(nn.Module):
    def __init__(self, 
        ...
        ```python
        # 使用微型 ViT 模型，适用于快速推理
        model = FastViT(model_name='vit_tiny_patch16_224', 
                       pretrained=True, 
                       num_classes=10)
        ```
        """
        ...
        # 通过优化器冻结部分层（可选，以提升训练速度）
        # 例如，冻结前 N 层
        # for name, param in self.vit.named_parameters():
        #     if 'blocks' in name and int(name.split('.')[2]) < 6:
...
if __name__ == "__main__":
    ...
    print("输出形状:", output.shape)  # 应输出 torch.Size([1, 10])