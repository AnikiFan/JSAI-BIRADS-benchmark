...
def modelSelector(model, lr, num_class):
    ...
    if model == 'TDSNet':
        ...
        model = UnetClassifier(num_classes=num_class, in_channels=cfg['in_channels'],
                               backbone=model_cfg[model]["backbone"], pretrained=model_cfg[model]["pretrained"])
        optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9)
    ...
    for epoch in range(cfg["epoch_num"]):
        ...
        if avg_vloss < best_vloss:
            ...
            torch.save(model.state_dict(),
                       modelCheckPoint_path + f'/{cfg["model"]}_ac{avg_vprecision:.6f}_f1{avg_vf1:.6f}_{epoch}.pth')
            ...
            torch.save(optimizer.state_dict(),
                       optimizerCheckPoint_path + f'/{cfg["model"]}_ac{avg_vprecision:.6f}_f1{avg_vf1:.6f}_{epoch}.pth')
